...
Tests use **moto** to mock AWS services — no real credentials needed. `tests/conftest.py` holds the autouse fake-credentials fixture and a mocked KMS key; each test module creates the DynamoDB tables and S3 buckets it needs inline. CI runs against Python 3.10, 3.11, 3.12.
