...
Tests use **moto** to mock AWS services — no real credentials needed. `tests/conftest.py` holds the shared fixtures: `aws_credentials` (autouse, fake creds) and `kms_key` (a mocked KMS key + alias). Test modules that need DynamoDB tables or S3 buckets build them inline, since the schemas differ per module. CI runs against Python 3.10, 3.11, 3.12.
